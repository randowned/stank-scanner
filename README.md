...
- **Media analytics** for YouTube and Spotify. Add videos/albums via the admin panel, view metric history and comparison charts on the dashboard, and query stats in Discord with `/stats youtube info <name>` / `/stats spotify info <name>` / `/stats youtube chart <name> [type] [range]` / `/stats youtube channel` / `/stats spotify artist`. Per-provider embed templates, scheduled metric snapshots, configurable refresh intervals, public chart image endpoint, and configurable milestone announcements (sparse K-scale: 1K, 5K, 10K, 25K, 50K, 100K, 250K, 500K, 750K; then every 1M views/plays through 50M then 75M / 100M / 150M / ... / 1B) — with celebratory embeds pushed to announcement channels. Channel/artist owner milestones (subscribers + total views for YouTube; followers + total plays for Spotify) are announced with the same pipeline. The `channel` / `artist` commands show owner-level stats (subscribers, video count + total-views / total-likes / total-comments aggregate for YouTube; followers, popularity + total-plays for Spotify). Spotify playcount is fetched via the Partner API using the bot owner's connected account (OAuth PKCE flow in Settings).
