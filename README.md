...
| `/stats youtube chart <name> <type> <range> [mode] [aggregation]` | Inline chart image (16:9 PNG) for views, likes, or comments. `mode` toggles total (default) vs per-tick delta. `aggregation` buckets data (5min/15min/hourly/daily/weekly/monthly). |
| `/stats spotify chart <name> <type> <range> [mode] [aggregation]` | Inline chart image for popularity. `mode` toggles total (default) vs per-tick delta. `aggregation` buckets data (5min/15min/hourly/daily/weekly/monthly). |

Admins can disable individual media providers per-guild from the media settings panel; non-admins won't see disabled-provider items, while admins always see everything (so they can re-enable). The dashboard detail page exposes a `Resolution` dropdown (5 Min / 15 Min / Hour / Day / Week / Month, with range-adaptive filtering) that time-buckets data by summing deltas within each bucket; the Discord chart-image endpoint supports the same aggregation buckets. Snapshot intervals are configurable from 1 min to 24 hours (default 60 min).
...
- `/media/{id}` — single media item detail: provider-aware metric tiles (1 tile for Spotify, 3 for YouTube), time-scale history chart with sub-day range options (1h / 6h / 12h / 1d / 7d / 30d / 90d / 1y, default 1 day), view mode toggle (Change / Cumulative) with time-bucket resampling in delta mode, resolution dropdown (Auto / Min / 5 Min / 15 Min / Hour / Day / Week / Month, range-filtered so buckets < range are hidden) with named tooltips, data-start indicator when data is shorter than selected range, multi-series comparison overlay (type-filtered), staleness pill in header, "Open on YouTube / Spotify" external link button, sparse-data hint when only one snapshot exists, gap-free line rendering.
- Admin `/admin/media` — manage media: add (tabbed by provider, optional name), client-side type filter (no API reload on tab switch), colored provider badges (YouTube red, Spotify green), force-refresh single or all, settings modal with interval dropdown (1min–24h) + ephemeral replies toggle + admin-only toggle, mobile-friendly rows with edge-to-edge thumbnails.
