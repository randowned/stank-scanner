...
| `/stats youtube chart <name> <type> <range> [mode] [aggregation]` | Inline chart image (16:9 PNG) for views, likes, or comments. `mode` toggles total (default) vs per-tick delta. `aggregation` buckets data (5min/15min/30min/hourly/daily/weekly/monthly); auto-selected when omitted. |
| `/stats spotify chart <name> <type> <range> [mode] [aggregation]` | Inline chart image for popularity. `mode` toggles total (default) vs per-tick delta. `aggregation` buckets data (5min/15min/30min/hourly/daily/weekly/monthly); auto-selected when omitted. |
