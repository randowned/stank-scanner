<script lang="ts">
	...
	const guildName = $derived((data as { guild_name?: string }).guild_name ?? '');
	const isLoading = $derived(!items || items.length === 0);
...
<PageHeader title={`Media: Maphra${guildName ? ` ${guildName}` : ''}`} subtitle="Media analytics" />
...
{#if isLoading}
	<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4" data-testid="media-skeleton">
		{#each Array(6) as _}
			<div class="panel animate-pulse">
				<Skeleton width="100%" height="160px" rounded="lg" />
				<div class="mt-3"><Skeleton width="80%" height="16px" /></div>
				<div class="mt-2"><Skeleton width="50%" height="12px" /></div>
			</div>
		{/each}
	</div>
